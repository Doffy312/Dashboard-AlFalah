...
const InventarisPage = () => {
  const { inventaris, summaries, addInventaris, updateInventaris, deleteInventaris } = useInventaris();
  ...
  // Takmir/Ketua: Read
  // Sekretaris: Read (PRD says Bendahara Full-access)
  // Bendahara: Full CRUD
  ...
  const confirmDelete = () => {
    if (inventarisToDelete) {
      deleteInventaris(inventarisToDelete.id);
  ...
  const handleSubmit = (data) => {
    if (editingInventaris) {
      updateInventaris(editingInventaris.id, data);
    } else {
      addInventaris(data);
    }
  };
