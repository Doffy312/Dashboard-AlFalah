...
const KanbanCard = ({ program, onEdit, onDelete, onStatusChange, canEdit }) => {
  ...
  return (
    <div className="bg-white/50 dark:bg-white/5 border border-white/40 dark:border-white/10 backdrop-blur-md rounded-xl p-md shadow-sm hover:shadow-md transition-all hover:-translate-y-1 group">
      <div className="flex justify-between items-start mb-sm">
        ...
        {canEdit && (
          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button onClick={() => onEdit(program)} className="p-1 rounded bg-white/30 dark:bg-white/10 hover:bg-white/50 text-on-surface-variant transition-colors">
              <Edit2 size={14} />
            </button>
      ...
      <div className="flex items-center justify-between mt-auto pt-sm border-t border-white/20 dark:border-white/10">
        ...
        {canEdit && nextStatus && (
          <button 
            ...
            className="flex items-center gap-1 text-[11px] font-label-sm px-2 py-1 rounded bg-white/30 dark:bg-white/10 hover:bg-primary hover:text-white text-on-surface-variant transition-colors"
        ...
        return (
          <div key={col.status} className={`flex-1 w-full min-h-[500px] rounded-2xl border ${col.colorClass} p-sm flex flex-col gap-sm`}>
            <div className="px-sm py-xs flex justify-between items-center border-b border-inherit mb-xs">
              ...
              <span className={`px-2 py-0.5 rounded-full text-[12px] font-bold bg-white/50 dark:bg-black/20 ${col.textClass}`}>
                {colPrograms.length}
              </span>
            </div>
            ...
            <div className="flex flex-col gap-sm overflow-y-auto max-h-[600px] pr-1 styled-scrollbar">
              ...
              {colPrograms.length === 0 && (
                <div className="py-xl text-center border-2 border-dashed border-white/20 dark:border-white/10 rounded-xl text-on-surface-variant/50">
