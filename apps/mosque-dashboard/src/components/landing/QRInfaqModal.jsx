...
const QRInfaqModal = ({ isOpen, onClose, defaultType = 'Infaq', onSuccessCallback }) => {
  ...
  return (
    <div 
      onClick={onClose}
      className="fixed inset-0 z-[100] flex items-start sm:items-center justify-center bg-slate-950/85 backdrop-blur-md p-4 sm:p-6 pt-16 sm:pt-8 overflow-y-auto animate-in fade-in duration-200"
    >
      <div 
        onClick={(e) => e.stopPropagation()}
        className="bg-[#0b131a] border border-emerald-500/30 p-6 sm:p-8 rounded-3xl max-w-lg w-full relative shadow-2xl my-auto text-white max-h-[90vh] overflow-y-auto flex flex-col gap-6"
      >
        {/* Sticky Header */}
        <div className="sticky top-0 z-20 bg-[#0b131a]/95 backdrop-blur-md flex justify-between items-start pt-1 pb-4 border-b border-white/10 -mt-2">
            ...
            <div className="pt-3 border-t border-white/10">
              ...
              {showSimulasiForm && (
                <form onSubmit={handleProcessDonateSimulasi} className="space-y-3 pt-3 animate-in fade-in duration-200">
                  <div>
                    <label className="text-xs font-semibold text-slate-300 block mb-1">Nama Donatur (Opsional)</label>
                    <input 
                      type="text"
                  ...
                  <div>
                    <label className="text-xs font-semibold text-slate-300 block mb-1">Catatan / Doa (Opsional)</label>
                    <input 
                      type="text"
